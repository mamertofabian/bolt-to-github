...
3. Refresh the Outcome index when needed and run
   `./scripts/maid recall --for-manifest manifests/drafts/<slug>.manifest.yaml --plan-packet`
...
5. Promote one implementation-sized draft with
   `./scripts/maid manifest promote manifests/drafts/<slug>.manifest.yaml`.
...
`./scripts/maid plan revise <manifest> --reason "<text>" --preserve-red-evidence`
...
`./scripts/maid plan revise <manifest> --reason "<text>" --stash-implementation`
...
`./scripts/maid insights` return related Outcome evidence, do not paste a raw
...
index with
`./scripts/maid learn --include-status completed --include-status abandoned` and
...
`./scripts/maid manifest promote manifests/drafts/<slug>.manifest.yaml`; do not
