...
const mockFactory = (() => {
  const { MockAuthenticationStrategyFactory } = jest.requireActual('./test-fixtures/UnifiedGitHubServiceFixtures');
...
describe('UnifiedGitHubService - Comprehensive Tests', () => {
  ...
  describe('Repository Operations', () => {
    ...
    it('should get commit count for repository', async () => {
      ...
      scenario.reset();
      const { mockFetch: newMockFetch } = scenario
        .setupSuccessfulPATAuthentication()
        .build();
  ...
  describe('Error Handling', () => {
    describe('Network Errors', () => {
      beforeEach(() => {
        ...
        mockFetch = jest.fn().mockRejectedValue(new Error('Network request failed'));
    ...
    describe('Rate Limiting', () => {
      beforeEach(() => {
        const mocks = scenario
          .setupRateLimiting('testuser', 'test-repo')
          .build();
        mockFetch = mocks.mockFetch;
      });
