...
const mockGitHubAppStrategy = new MockGitHubAppAuthenticationStrategy(TestFixtures.TokenFixtures.oauth.accessToken);

const mockFactory = {
  createPATStrategy: jest.fn((token: string) => {
    const strategy = new (jest.requireActual('./test-fixtures/UnifiedGitHubServiceFixtures')).MockPATAuthenticationStrategy(token);
...
describe('UnifiedGitHubService - Focused Tests', () => {
  ...
  describe('Constructor and Basic Functionality', () => {
    ...
    it('should create instance with auth config', () => {
      const authConfig = UnifiedGitHubServiceTestHelpers.createAuthConfig('pat', TestFixtures.TokenFixtures.pat.classic);
  ...
  describe('Authentication - Failure Cases', () => {
    beforeEach(() => {
      mockFetch
        .mockUnauthorized('GET:https://api.github.com/repos/testuser/test-repo')
        .build();
    });
  ...
  describe('Feedback System', () => {
    beforeEach(() => {
      mockFetch
        .mockCreateIssue('mamertofabian', 'bolt-to-github')
        .build();
    });
