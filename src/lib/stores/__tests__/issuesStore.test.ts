...
describe('issuesStore', () => {
  beforeEach(() => {
    ...
    mockCreateConnectedGitHubAppService.mockResolvedValue(mockUnifiedGitHubService);
  });

  it('issue store uses GitHub App authentication without token arguments', async () => {
    const issue = createMockGitHubIssue();
    mockUnifiedGitHubService.getIssues.mockResolvedValue([issue]);
    mockUnifiedGitHubService.createIssue.mockResolvedValue(issue);
    mockUnifiedGitHubService.updateIssue.mockResolvedValue({ ...issue, state: 'closed' });

    await issuesStore.loadIssues('app-owner', 'app-repo', 'all', true);
    await issuesStore.createIssue('app-owner', 'app-repo', { title: 'App issue' });
    await issuesStore.updateIssue('app-owner', 'app-repo', 1, { state: 'closed' });

    expect(mockCreateConnectedGitHubAppService).toHaveBeenCalledTimes(3);
    ...
    expect(mockUnifiedGitHubService.createIssue).toHaveBeenCalledWith('app-owner', 'app-repo', {
      title: 'App issue',
    });
  ...
  it('issue operations fail visibly when live connection is unavailable', async () => {
    ...
    await expect(
      issuesStore.loadIssues('disconnected-owner', 'disconnected-repo', 'open', true)
    ).rejects.toThrow(connectionMessage);

    expect(get(issuesStore.getIssuesForRepo('disconnected-owner', 'disconnected-repo')).error).toBe(
      connectionMessage
    );

    await expect(
      issuesStore.createIssue('create-owner', 'create-repo', { title: 'Disconnected create' })
    ).rejects.toThrow(connectionMessage);
    expect(get(issuesStore.getIssuesForRepo('create-owner', 'create-repo'))).toMatchObject({
      issues: [],
      isLoading: false,
      error: connectionMessage,
      lastFetched: 0,
    });
    expect(get(issuesStore.getOpenIssuesCount('create-owner', 'create-repo'))).toBe(0);

    await expect(
      issuesStore.updateIssue('update-owner', 'update-repo', 1, { state: 'closed' })
    ).rejects.toThrow(connectionMessage);
    expect(get(issuesStore.getIssuesForRepo('update-owner', 'update-repo'))).toMatchObject({
      issues: [],
      isLoading: false,
      error: connectionMessage,
      lastFetched: 0,
    });
    expect(get(issuesStore.getOpenIssuesCount('update-owner', 'update-repo'))).toBe(0);
  });
  ...
  describe('loadIssues', () => {
    it('should load all issues from GitHub API', async () => {
      ...
      const issues = await issuesStore.loadIssues('owner', 'repo', 'all');
    ...
    it('should filter open issues when state is open', async () => {
      ...
      const issues = await issuesStore.loadIssues('owner', 'repo', 'open');
    ...
    it('should filter closed issues when state is closed', async () => {
      ...
      const issues = await issuesStore.loadIssues('owner', 'repo', 'closed');
    ...
    it('should use cached data when cache is valid', async () => {
      ...
      await issuesStore.loadIssues('owner', 'repo', 'all');

      vi.clearAllMocks();

      const cachedIssues = await issuesStore.loadIssues('owner', 'repo', 'all');
    ...
    it('should force refresh when forceRefresh is true', async () => {
      ...
      await issuesStore.loadIssues('owner', 'repo', 'all', false);

      const freshIssues = [createMockGitHubIssue({ number: 2 })];
      mockUnifiedGitHubService.getIssues.mockResolvedValue(freshIssues);

      const issues = await issuesStore.loadIssues('owner', 'repo', 'all', true);
    ...
    it('should handle errors and update error state', async () => {
      ...
      await expect(issuesStore.loadIssues('owner', 'repo', 'all')).rejects.toThrow('API Error');
    ...
    it('should preserve existing issues on error', async () => {
      ...
      await expect(issuesStore.loadIssues('owner', 'repo', 'all')).rejects.toThrow();
    ...
    it('should handle multiple repositories independently', async () => {
      ...
      await issuesStore.loadIssues('owner1', 'repo1', 'all');
      await issuesStore.loadIssues('owner2', 'repo2', 'all');
  ...
  describe('createIssue', () => {
    it('should create a new issue and add it to the store', async () => {
      ...
      const result = await issuesStore.createIssue('owner', 'repo', {
        title: 'New Issue',
        body: 'Issue body',
      });
    ...
    it('should add new issue to the beginning of existing issues', async () => {
      ...
      await issuesStore.createIssue('owner', 'repo', {
        title: 'Second Issue',
      });
    ...
    it('should handle create errors and update error state', async () => {
      ...
      await expect(issuesStore.createIssue('owner', 'repo', { title: 'Test' })).rejects.toThrow(
        'Create failed'
      );
    ...
    it('should create issue in repository with no existing issues', async () => {
      ...
      await issuesStore.createIssue('owner', 'repo', { title: 'First Issue' });
    ...
    it('should update lastFetched timestamp when creating', async () => {
      ...
      await issuesStore.createIssue('owner', 'repo', { title: 'Test' });
  ...
  describe('updateIssue', () => {
    it('should update an existing issue in the store', async () => {
      ...
      const result = await issuesStore.updateIssue('owner', 'repo', 1, {
        title: 'Updated',
      });
    ...
    it('should close an issue', async () => {
      ...
      await issuesStore.updateIssue('owner', 'repo', 1, { state: 'closed' });
    ...
    it('should reopen a closed issue', async () => {
      ...
      await issuesStore.updateIssue('owner', 'repo', 1, { state: 'open' });
    ...
    it('should update multiple fields at once', async () => {
      ...
      await issuesStore.updateIssue('owner', 'repo', 1, {
        title: 'Updated Title',
        body: 'Updated body',
        state: 'closed',
      });
    ...
    it('should schedule a force refresh after updating', async () => {
      ...
      await issuesStore.updateIssue('owner', 'repo', 1, { title: 'Updated' });
    ...
    it('should handle update errors and update error state', async () => {
      ...
      await expect(issuesStore.updateIssue('owner', 'repo', 1, { title: 'Test' })).rejects.toThrow(
        'Update failed'
      );
    ...
    it('should preserve other issues when updating one', async () => {
      ...
      await issuesStore.updateIssue('owner', 'repo', 2, { title: 'Updated Second' });
    ...
    it('should not modify store if issue number not found', async () => {
      ...
      await issuesStore.updateIssue('owner', 'repo', 999, { title: 'Test' });
  ...
  describe('getIssuesForRepo', () => {
    ...
    it('should return reactive store that updates when issues change', async () => {
      ...
      const newIssue = createMockGitHubIssue({ number: 2 });
      mockUnifiedGitHubService.createIssue.mockResolvedValue(newIssue);
      await issuesStore.createIssue('owner', 'repo', { title: 'New' });
  ...
  describe('getLoadingState', () => {
    ...
    it('should track loading state during async operations', async () => {
      ...
      await issuesStore.loadIssues('owner', 'repo', 'open');
  ...
  describe('clearError', () => {
    ...
    it('should not affect other repositories when clearing error', async () => {
      ...
      await expect(issuesStore.loadIssues('owner1', 'repo1', 'all')).rejects.toThrow();

      mockUnifiedGitHubService.getIssues.mockRejectedValue(new Error('Error 2'));
      await expect(issuesStore.loadIssues('owner2', 'repo2', 'all')).rejects.toThrow();
  ...
  describe('edge cases and error handling', () => {
    ...
    it('should handle rapid successive calls to loadIssues', async () => {
      ...
      const promise1 = issuesStore.loadIssues('owner', 'repo', 'all', true);
      const promise2 = issuesStore.loadIssues('owner', 'repo', 'all', true);
      const promise3 = issuesStore.loadIssues('owner', 'repo', 'all', true);
    ...
    it('should handle empty repository name', async () => {
      ...
      const issues = await issuesStore.loadIssues('owner', '', 'all');
    ...
    it('should handle special characters in owner and repo names', async () => {
      ...
      await issuesStore.loadIssues('owner-123', 'repo_name.test', 'all');
    ...
    it('should acquire a fresh GitHub App service for a forced refresh', async () => {
      const mockIssues = [createMockGitHubIssue({ number: 1 })];
      mockUnifiedGitHubService.getIssues.mockResolvedValue(mockIssues);
      ...
      await issuesStore.loadIssues('owner', 'repo', 'all', true);

      expect(mockUnifiedGitHubService.getIssues).toHaveBeenCalledTimes(2);
      expect(mockCreateConnectedGitHubAppService).toHaveBeenCalledTimes(2);
    });
    ...
    it('should handle concurrent operations on different repositories', async () => {
      ...
      await Promise.all([
        issuesStore.loadIssues('owner1', 'repo1', 'all'),
        issuesStore.loadIssues('owner2', 'repo2', 'all'),
      ]);

      await Promise.all([
        issuesStore.createIssue('owner1', 'repo1', { title: 'Issue 1' }),
        issuesStore.createIssue('owner2', 'repo2', { title: 'Issue 2' }),
      ]);
  ...
  describe('integration scenarios', () => {
    it('should handle complete issue lifecycle', async () => {
      ...
      await issuesStore.createIssue('owner', 'repo', { title: 'New Issue' });
    ...
    it('should maintain separate state across multiple projects', async () => {
      ...
      await issuesStore.loadIssues('owner', 'repo1', 'all');
      await issuesStore.loadIssues('owner', 'repo2', 'all');
      ...
      await expect(issuesStore.loadIssues('owner', 'repo1', 'all', true)).rejects.toThrow();
