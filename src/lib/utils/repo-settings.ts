...
/**
 * Handles keyboard navigation for branch selection with create option
 * @param key - Keyboard key pressed
 * @param currentIndex - Current selected index
 * @param filteredBranches - Array of filtered branch names
 * @param showCreateOption - Whether "Create new branch" option is shown
 * @returns Object with navigation result
 */
export function handleBranchKeyboardNavigation(
  key: string,
  currentIndex: number,
  filteredBranches: string[],
  ...
  // Build items array with optional __CREATE__ marker
  const items = showCreateOption ? ['__CREATE__', ...filteredBranches] : filteredBranches;
